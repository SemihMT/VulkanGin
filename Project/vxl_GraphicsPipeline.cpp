...
namespace vxl
{
	...
	vxlGraphicsPipeline::vxlGraphicsPipeline(vxlDevice& device, const std::string& vertFilePath, const std::string& fragFilePath, const PipelineConfiguration& config) :
		m_device{ device }
	{
		CreateGraphicsPipeline(vertFilePath, fragFilePath, config);
	}

	vxlGraphicsPipeline::~vxlGraphicsPipeline()
	{
	...
	void vxlGraphicsPipeline::Bind(VkCommandBuffer commandBuffer)
	{
		vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipeline);
	}

	void vxlGraphicsPipeline::CreateDefaultPipelineConfiguration(PipelineConfiguration& config)
	{
	...
	std::vector<char> vxlGraphicsPipeline::ReadFile(const std::string& filePath)
	{
	...
	void vxlGraphicsPipeline::CreateGraphicsPipeline(const std::string& vertFilePath, const std::string& fragFilePath, const PipelineConfiguration& config)
	{
		...
		const auto bindingDescriptions = vxlModel::Vertex::GetBindingDescriptions();
		const auto attributeDescriptions = vxlModel::Vertex::GetAttributeDescriptions();
	...
	void vxlGraphicsPipeline::CreateShaderModule(const std::vector<char>& shaderCode, VkShaderModule* shaderModule)
	{
